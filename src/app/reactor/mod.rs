...
mod SpaceEventHandler {
    ...
    pub fn handle_window_server_destroyed(
        ...
        let assigned_space =
            tracked_window.and_then(|window| reactor.affinity().assigned_space_for_window_id(window));
        let observations = super::events::space::WindowServerDestroyedObservations {
            resolved_space: reactor.affinity().resolve_native_space(wsid, None),
            active_spaces: reactor.active_spaces.clone(),
            mission_control_active: reactor.is_mission_control_active(),
            ordered_in: crate::windows::platform::window_server::window_ordered_in(wsid),
            assigned_space,
            last_known_user_space: super::events::space::resolve_last_known_user_space(
                tracked_window.and_then(|window| reactor.affinity().best_space_for_window_id(window)),
    ...
    pub fn handle_window_server_appeared(
        ...
        kind: crate::displays::domain::topology::SpaceEventKind,
    ) {
...
use crate::displays::domain::topology::{ForwardedSpaceState, SpaceEventKind, TopologyWindowDelta};
...
use crate::workspaces::{self as layout, Direction, LayoutEngine, LayoutEvent};
use crate::workspaces::broadcast::{BroadcastEvent, BroadcastSender, protocol_window_id, protocol_workspace_id};
...
pub use crate::displays::screen::ScreenInfo;
...
pub(crate) use crate::app::reactor::state::AppState;
pub(crate) use crate::windows::domain::state::WindowFilter;
...
pub enum Event {
    ...
    #[serde(skip)]
    RegisterWmSender(crate::app::hotkeys::Sender),
...
impl From<crate::displays::event::Event> for Event {
    fn from(event: crate::displays::event::Event) -> Self {
        use crate::displays::event::Event as D;
        match event {
...
/// `crate::windows::event::Event` into the reactor's channel through this.
impl From<crate::windows::event::Event> for Event {
    fn from(event: crate::windows::event::Event) -> Self {
...
impl Reactor {
    pub fn spawn(
        ...
        let (events_tx, events) = channels::channel();
    ...
    /// The read-only space-resolution view over the stores. See `space_affinity.rs`.
    pub(crate) fn affinity(&self) -> SpaceAffinity<'_> {
        SpaceAffinity {
    ...
    fn apply_app_rules_for_activated_spaces(&mut self, activated: &[SpaceId]) {
        ...
        for (wid, state) in self.state.windows.iter_windows() {
            ...
            let Some(space) = self.affinity().best_space_for_window_id(wid) else {
                continue;
            };
    ...
    fn authoritative_active_space_windows(&self) -> Vec<(WindowServerId, Option<SpaceId>)> {
        ...
        let mut membership: Vec<_> = membership
            .into_iter()
            .map(|(wsid, space)| (wsid, self.affinity().resolve_native_space(wsid, Some(space))))
            .collect();
    ...
    fn has_known_windows_for_active_spaces(&self) -> bool {
        self.state.windows.iter_windows().any(|(wid, _)| {
            self.affinity().authoritative_space_for_window_id(wid)
                .is_some_and(|space| self.is_space_active(space))
        })
    }
    ...
    fn refresh_active_space_window_membership(
        ...
        for (wsid, space) in active_windows {
            let space = self.affinity().resolve_native_space(wsid, space);
    ...
    fn remove_windows_missing_from_active_space_snapshot(
        ...
        for wsid in previously_visible_wsids {
            ...
            let Some(space) = self.affinity().assigned_space_for_window_id(wid) else {
                continue;
            };
    ...
    fn clear_pending_hidden_window_targets(&self) {
        for (wid, window) in self.state.windows.iter_windows() {
            if self.affinity().hidden_assigned_space_for_window_id(wid).is_none() {
                continue;
            }
    ...
    fn clear_pending_target_if_confirmed_space(
        ...
        if self.affinity().pending_target_space_for_window_server_id(wsid) == Some(confirmed_space) {
            self.transaction_manager.clear_target_for_window(wsid);
        }
    }
    ...
    async fn run(mut reactor: Reactor, events: Receiver, events_tx: Sender) {
        let (raise_manager_tx, raise_manager_rx) = channels::channel();
        ...
        let warp = move |point| {
            if let Some(event_tap_tx) = &event_tap_tx {
                _ = event_tap_tx.send(crate::input::platform::input_tap::Request::Warp(point));
            }
        };
    ...
    fn dispatch_workflow(&mut self, event: Event) -> anyhow::Result<EventOutcome> {
        ...
        match event {
            ...
            Event::WindowServerDestroyed(wsid, sid, kind) => {
                ...
                let assigned_space =
                    tracked_window.and_then(|window| self.affinity().assigned_space_for_window_id(window));
            ...
            Event::WindowServerAppeared(wsid, sid, kind) => {
                ...
                let observations = topology_workflow::WindowServerAppearedObservations {
                    is_parked_by_rini,
                    resolved_space: self.affinity().resolve_native_space(wsid, Some(sid)),
                    active_spaces: self.active_spaces.clone(),
                    mission_control_active: self.is_mission_control_active(),
                    assigned_space,
                    last_known_user_space,
            ...
            Event::WindowFrameChanged(wid, new_frame, last_seen, requested, mouse_state) => {
                ...
                if matches!(
                    ...
                    outcome.dispatch_mouse_up = effective_mouse_state
                        == Some(crate::windows::platform::mouse::MouseState::Up)
                        && matches!(
                            self.drag_manager.drag_state,
                            DragState::Active { .. } | DragState::PendingSwap { .. }
                        );
                ...
                let best_resize_space = self.affinity().best_space_for_window(&new_frame, server_id);
                ...
                let pending_target_space = server_id
                    .and_then(|server| self.affinity().pending_target_space_for_window_server_id(server));
                let assigned_space = self.affinity().assigned_space_for_window_id(wid);
                ...
                if effective_mouse_state == Some(crate::windows::platform::mouse::MouseState::Up)
                    && matches!(
                        self.drag_manager.drag_state,
                        DragState::Active { .. } | DragState::PendingSwap { .. }
                    )
    ...
    fn apply_event_outcome(&mut self, outcome: EventOutcome) {
        ...
        if let Some((space, window)) =
            focus_service::resolve(outcome.focused_window, |wid| self.affinity().best_space_for_window_id(wid))
        {
        ...
        if let Some(direction) = outcome.switch_native_space {
            unsafe { crate::displays::platform::space_switch::switch_space(direction) };
        }
        ...
        if let Some(config) = outcome.service_config_update {
            if let Some(wm) = &self.communication_manager.wm_sender {
                wm.send(crate::app::hotkeys::WmEvent::ConfigUpdated(config));
            }
        ...
        if let Some(wid) = self.workspace_switch_manager.pending_workspace_mouse_warp.take() {
            if let Some(window_center) = self.affinity().window_center_on_known_screen(wid) {
    ...
    fn update_partial_window_server_info(&mut self, ws_info: Vec<WindowServerInfo>) {
        ...
        for info in ws_info.iter() {
            ...
            if let Some(wid) = self.state.windows.tracked_window_id(info.id) {
                ...
                let manageable = crate::windows::platform::window_server::compute_window_manageability(
                    server_id,
                    is_minimized,
                    is_ax_standard,
                    is_ax_root,
            ...
            for record in records {
                ...
                if let (Some(window_id), Some(target_space)) = (live_window_id, target_space)
                    && let Some(source_space) =
                        self.affinity().best_space_for_window_id(window_id).or(Some(target_space))
    ...
    fn maybe_reapply_app_rules_for_window(&mut self, window_id: WindowId) {
        ...
        let Some(space) = self.affinity().best_space_for_window_id(window_id) else {
            return;
        };
        if !self.is_space_active(space) {
            return;
    ...
    fn handle_authoritative_space_snapshot(
        ...
        let analysis = crate::displays::domain::topology::analyze_space_snapshot(
            &self.space_state,
            &self.active_spaces,
        ...
        let crate::displays::domain::topology::SpaceSnapshotAnalysis {
            spaces,
    ...
    fn on_windows_discovered_with_app_info(
        ...
        let inactive_windows = self
            ...
            .iter_windows()
            .filter_map(|(wid, _)| {
                (wid.pid == pid && self.affinity().is_window_on_known_inactive_space(wid)).then_some(wid)
            })
            .collect();
        ...
        let observed_windows = new
            ...
            .map(|(wid, info)| {
                let current_native_space =
                    info.sys_id.and_then(|wsid| self.affinity().resolve_native_space(wsid, None));
                let active_space = self
                    .affinity().best_space_for_window(&info.frame, info.sys_id)
                    .filter(|space| self.is_space_active(*space))
                    .or_else(|| {
        ...
        let discovery_spaces = candidate_windows
            .iter()
            .filter_map(|wid| self.affinity().discovery_space_for_window_id(*wid).map(|space| (*wid, space)))
            .collect();
        let authoritative_spaces = candidate_windows
            .iter()
            .filter_map(|wid| {
                self.affinity().authoritative_space_for_window_id(*wid).map(|space| (*wid, space))
            })
            .collect();
    ...
    fn ensure_active_drag(&mut self, wid: WindowId, frame: &CGRect) {
        ...
        if needs_new_session {
            ...
            let origin_space = self.affinity().best_space_for_window(frame, server_id);
    ...
    fn apply_topology_window_delta(&mut self, delta: TopologyWindowDelta) -> EventOutcome {
        ...
        for window_server_id in window_server_ids {
            ...
            let authoritative_space = self.affinity().resolve_native_space(window_server_id, appeared_space);
            if let Some(target_space) = authoritative_space {
                ...
                self.state.windows.mark_window_hidden(window_server_id);
                if let Some(window) = self.state.windows.tracked_window_id(window_server_id)
                    && self.affinity().assigned_space_for_window_id(window) == Some(previous_space)
    ...
    fn restore_fullscreen_window_to_user_space(
        ...
        if owner != original_window && self.affinity().assigned_space_for_window_id(original_window).is_some()
        {
        ...
        *outcome = std::mem::take(outcome).with_app_request(owner.pid, Request::GetVisibleWindows);
        Some(if self.affinity().assigned_space_for_window_id(owner) == Some(space) {
            self.is_space_active(space)
    ...
    fn redistribute_windows(&mut self) -> EventOutcome {
        ...
        for (display_uuid, target_space) in &attached {
            ...
            for window in windows {
                let Some(source_space) = self.affinity().assigned_space_for_window_id(window) else {
                    continue;
                };
    ...
    fn reassign_window_to_authoritative_space_with_workspace_preservation(
        ...
        {
            let changed_space = self.affinity().assigned_space_for_window_id(wid);
            self.send_layout_event(LayoutEvent::WindowRemoved(wid));
    ...
    fn reconcile_windows_with_authoritative_spaces(&mut self) -> bool {
        ...
        for wid in windows {
            let Some(authoritative_space) = self.affinity().authoritative_space_for_window_id(wid) else {
                continue;
            };
    ...
    fn publish_cursor_warp_screens(&self) {
        ...
        _ = tx.send(crate::displays::platform::cursor_warp::Request::SetEnabled(
            self.config.settings.warp_cursor_between_stacked_displays,
        ));
        _ = tx.send(crate::displays::platform::cursor_warp::Request::SetUpperSide(
            self.config.settings.stacked_display_upper_is,
        ));
        _ = tx.send(crate::displays::platform::cursor_warp::Request::SetLowerTopAt(
            self.config.settings.stacked_display_lower_top_at,
        ));
        _ = tx.send(crate::displays::platform::cursor_warp::Request::ScreensChanged(
            crate::displays::platform::cursor_warp::screens_of(&self.space_state.screens),
        ));
    }
    ...
    fn apply_overlay_frames(&mut self, frames: Vec<(WindowId, CGRect)>) {
        ...
        for (wid, frame) in crate::animation::domain::motion::frame_writes::frame_send_order(frames, display) {
            ...
            if !crate::animation::domain::motion::frame_writes::frame_write_needed(real, frame, display) {
                skipped += 1;
                continue;
            }
    ...
    fn warm_all_workspaces(&mut self, space: SpaceId) {
        ...
        _ = tx.send(crate::animation::platform::engine::Event::WarmWindows(targets));
    }
    ...
    fn strip_windows_at(
        ...
        for (wid, frame) in layout {
            ...
            windows.push(crate::animation::platform::engine::SurfaceWindow {
                window: *wid,
                server_id,
                frame: CGRect::new(
                    CGPoint::new(
    ...
    fn start_edge_bounce(&mut self, space: SpaceId, direction: Direction) {
        ...
        let overshoot = crate::animation::domain::motion::plan::edge_bounce_overshoot(direction);
        let duration =
            std::time::Duration::from_secs_f64(self.config.settings.animation_duration.max(0.0));
        tracing::debug!(?direction, windows = windows.len(), "edge bounce");
        self.publish_animation_display_for(Some(space));
        _ = tx.send(crate::animation::platform::engine::Event::Bounce {
            windows,
            overshoot,
            final_frames: layout,
            focus: self.layout_manager.layout_engine.focused_window(),
    ...
    fn start_strip_switch(
        ...
        for index in low..=high {
            ...
            let row = crate::animation::domain::motion::strip_stack::row_of(index, from_index, to_index);
            for (wid, frame) in layout {
                let Some(window) = self.state.windows.window(wid) else { continue };
                let Some(server_id) = window.info.sys_id else { continue };
                windows.push(crate::animation::platform::engine::SurfaceWindow {
                    window: wid,
                    server_id,
                    frame: crate::animation::domain::motion::strip_stack::strip_frame(
                        frame,
        ...
        let travel = crate::animation::domain::motion::strip_stack::travel(from_index, to_index, height);
        ...
        if let Some(target) = self.layout_manager.layout_engine.focused_window()
            ...
            && !crate::animation::domain::motion::strip_stack::lands_in_view(placed.frame, display_bounds.size.width)
        {
            tracing::debug!(
                idx = target.idx.get(),
    ...
    pub(crate) fn publish_animation_display_for(&self, space: Option<SpaceId>) {
        ...
        _ = tx.send(crate::animation::platform::engine::Event::SetDisplay {
            id: screen.id.as_u32(),
    ...
    fn send_layout_event(&mut self, event: LayoutEvent) {
        ...
        if focus_changed && let Some(event_tap_tx) = &self.communication_manager.event_tap_tx {
            _ = event_tap_tx.send(crate::input::platform::input_tap::Request::HideOnFocus);
        }
    ...
    fn apply_app_rule_placements(
        &mut self,
        placements: Vec<crate::workspaces::domain::app_rules::AppRulePlacement>,
    ) {
    ...
    fn apply_app_rule_resizes(&mut self, resizes: Vec<crate::workspaces::domain::app_rules::AppRuleResize>) {
    ...
    pub(crate) fn should_raise_on_mouse_over(&self, wid: WindowId) -> bool {
        ...
        let Some(space) = self.affinity().best_space_for_window(&candidate_frame, window.info.sys_id) else {
            return false;
        };
        if !self.is_space_active(space) {
            return false;
        ...
        let order = crate::windows::platform::window_server::space_window_list_for_connection(&[space.get()], 0, false);
    ...
    fn handle_app_activation_workspace_switch(&mut self, pid: pid_t) -> EventOutcome {
        ...
        let Some(window_space) = self.affinity().best_space_for_window_id(app_window_id) else {
            return EventOutcome::no_change();
        };
    ...
    fn activation_redirect(&mut self, picked: WindowId, picked_space: SpaceId) -> Option<EventOutcome> {
        ...
        let target = crate::windows::domain::focus::activation_focus_target(
            picked,
        ...
        let space = self.affinity().best_space_for_window_id(target);
        let mut outcome =
    ...
    fn window_is_in_active_workspace(&self, window: WindowId, space: Option<SpaceId>) -> bool {
        let Some(space) = space.or_else(|| self.affinity().best_space_for_window_id(window)) else {
            return false;
        };
        self.is_space_active(space)
    ...
    fn handle_layout_response(
            ...
            if let Some(cmd) = cmd {
                ...
                if let Some(space) = space {
                    ...
                    if self.config.settings.gestures.haptics_enabled {
                        let _ = crate::input::platform::haptics::perform_haptic(
            ...
            && let Some(wsid) = state.info.sys_id
        {
            ...
            let best_space = self.affinity().best_space_for_window_state(state);
            if !is_visible {
                focus_window = None;
                if let Some(space) = workspace_switch_space
                    && !self.is_in_drag()
        ...
        let focus_window_with_warp = focus_window.map(|wid| {
            let warp = if self.config.settings.mouse_follows_focus {
                    ...
                    == WorkspaceSwitchState::Active
                {
                    ...
                    self.affinity().window_center_on_known_screen(wid)
                }
            } else {
                None
            };
    ...
    fn stack_group_of(&self, window: WindowId) -> crate::animation::domain::motion::z_group::StackGroup {
    ...
    fn strip_group_to_lift(&mut self, space: SpaceId, focused: WindowId) -> Vec<WindowId> {
        ...
        let mut order: Vec<StackedWindow> = self
            ...
            .windows_in_active_workspace(&self.state.windows, space)
            .into_iter()
            .filter(|wid| !self.affinity().is_window_parked_offscreen(*wid))
            .filter_map(|wid| {
    ...
    fn regroup_after_layout(&mut self) {
        ...
        let Some(space) = self.affinity().best_space_for_window_id(focused) else {
            return;
        };
    ...
    fn regroup_raise(
        ...
        for wid in to_raise {
            self.insert_app_handle_for_window(&mut app_handles, *wid);
            by_app.entry((wid.pid, self.affinity().best_space_for_window_id(*wid))).or_default().push(*wid);
        }
        if by_app.is_empty() {
            return None;
        }
    ...
    /// See [`crate::windows::domain::focus::RaiseEcho`].
    ...
    fn collect_drag_swap_candidates(
        ...
        self.state
            ...
            .filter_map(|(other_wid, other_state)| {
                ...
                let other_space = self.affinity().best_space_for_window_state(other_state)?;
    ...
    fn maybe_swap_on_drag(&mut self, wid: WindowId, new_frame: CGRect) {
        ...
        let Some(space) = self
            ...
            .or_else(|| self.affinity().best_space_for_window(&new_frame, server_id))
        else {
            return;
        };
        ...
        let origin_space_hint = self
            ...
            .or_else(|| {
                self.drag_manager
                    .origin_frame()
                    .and_then(|frame| self.affinity().best_space_for_window(&frame, server_id))
            });
    ...
    fn focus_desktop_if_active_workspace_empty(&mut self, space: SpaceId) -> bool {
        ...
        if !crate::displays::screen::focus_desktop_window(screen) {
            return false;
        }
    ...
    fn last_focused_window_in_space(&self, space: SpaceId) -> Option<WindowId> {
        ...
        if self.affinity().best_space_for_window_id(wid)? != space {
            return None;
        }
        if window
            .info
    ...
    fn visible_focus_candidate_in_active_workspace(
        ...
        let in_space = |wid: WindowId| {
            self.state.windows.window(wid).is_some_and(|w| w.info.sys_id.is_some())
                && self.affinity().best_space_for_window_id(wid) == Some(space)
                && self.layout_manager.layout_engine.is_window_in_active_workspace(
                    &self.state.windows,
                    space,
                    wid,
    ...
    fn request_refocus_if_hidden(&mut self, space: SpaceId, window_id: WindowId) {
        if self.affinity().window_in_non_active_workspace(space, window_id) {
            self.refocus_manager.refocus_state = RefocusState::Pending(space);
        }
    }
        ...
        match event {
            ...
            LayoutEvent::WindowsOnScreenUpdated(space, _, windows, _) => {
                let hidden_exists = windows.iter().any(|(wid, _, _, _, _, _, _, _, _)| {
                    self.affinity().window_in_non_active_workspace(*space, *wid)
                });
                if hidden_exists {
                    self.refocus_manager.refocus_state = RefocusState::Pending(*space);
                }
    ...
    fn command_context_space(&self) -> Option<SpaceId> {
        self.workspace_command_space().or_else(|| {
            self.layout_manager
                ...
                .focused_window()
                .and_then(|wid| {
                    self.affinity().assigned_space_for_window_id(wid)
                        .or_else(|| self.affinity().best_space_for_window_id(wid))
                })
                .filter(|space| self.is_space_active(*space))
...
pub(crate) struct StackedWindow {
    ...
    pub(crate) group: crate::animation::domain::motion::z_group::StackGroup,
}
...
pub(crate) fn strip_group_to_lift_for(
    order: &[StackedWindow],
    focused: WindowId,
    focused_group: crate::animation::domain::motion::z_group::StackGroup,
) -> Vec<WindowId> {
    ...
    let back_to_front = crate::animation::domain::motion::z_group::regroup_tiled(&groups);
