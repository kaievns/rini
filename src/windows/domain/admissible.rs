...
mod tests {
    ...
    fn ok() -> Candidate<'static> {
        Candidate { has_visible_peer: true, ..Default::default() }
    }
    ...
    fn a_window_the_window_server_cannot_see_is_turned_away() {
        let c = Candidate { has_visible_peer: false, ..Default::default() };
    ...
    fn a_minimized_window_is_admitted_without_a_visible_peer() {
        let c = Candidate { has_visible_peer: false, is_minimized: true, ..Default::default() };
        assert_eq!(rejection(&c), None);
    }

    #[test]
    fn a_widget_bundle_is_turned_away_in_both_of_its_forms() {
        for id in ["com.example.widget", "com.example.widget.extra", "COM.EXAMPLE.WIDGET"] {
    ...
    fn an_application_named_after_widgets_is_not_a_widget() {
        for id in ["com.example.widgetstudio", "com.widgets.app", "com.example.WidgetMaker"] {
            let c = Candidate { bundle_id: Some(id), ..ok() };
    ...
    fn an_ordinary_path_is_admitted() {
        let c = Candidate { path: Some("/Applications/Ghostty.app"), ..ok() };
        assert_eq!(rejection(&c), None);
    }
    ...
    fn popovers_and_menus_are_chrome_rather_than_windows() {
        ...
        let c = Candidate { ax_role: Some("AXWindow"), ..ok() };
        assert_eq!(rejection(&c), None);
    }
    ...
    fn a_window_with_no_server_id_yet_counts_as_visible() {
        assert!(has_visible_peer(false, false), "no id yet, so nothing to contradict AX");
        assert!(has_visible_peer(false, true));
        assert!(has_visible_peer(true, true));
        assert!(!has_visible_peer(true, false), "an id the server does not report is not on screen");
    }
    ...
    fn only_the_two_known_liars_need_a_title_element() {
        ...
        assert!(!needs_title_element_to_be_standard(Some("com.mitchellh.ghostty")));
