...
//! per rigid piece of the flight. See "The overlay engine" in `docs/animation-smoothness.md` and
//! the capture measurements in `docs/capture-overlay-research.md`.
...
/// See "Level and coverage" in `docs/capture-overlay-research.md`.
...
/// see "Shadows are never in the surface" in `docs/capture-overlay-research.md`.
...
/// pops at the lift. See "Real windows land before lift" in `docs/animation-smoothness.md`.
...
/// reactor's next synchronous calls. See "The overlay engine" in `docs/animation-smoothness.md`.
fn commit_now() {
...
/// the tile. See "A layer shadow covers the whole layer" in `docs/capture-overlay-research.md`.
struct Tile {
    ...
    /// Hairline sublayers with their index in [`crate::edge_dressing::DressingLayout`] order
...
pub struct TileOverlay {
    ...
    /// bar has to be captured on its own" in `docs/capture-overlay-research.md`.
...
impl TileOverlay {
    ...
    /// full bounds in CoreGraphics coordinates. See "Level and coverage" in `docs/capture-overlay-research.md`.
    ...
    pub fn set_tile_picture(
        ...
        if entry.crop_of.is_some() {
            ...
            // "Window borders during animations" in `docs/animation-smoothness.md`.
    ...
    pub fn set_tile_dressing(
        &mut self,
        window: WindowId,
        dressing: &crate::edge_dressing::EdgeDressing,
    ) {
        let scale = self.scale;
        let Some(entry) = self.tile_layers.get_mut(&window) else { return };
        CATransaction::begin();
    ...
    /// `Floating`. Pre-flight only; see "The overlay engine" in `docs/animation-smoothness.md`.
    ...
    /// See "Edge bounce" in `docs/animation-smoothness.md`.
    ...
    /// render server runs behind the actor's clock. See "Real windows land before lift" in `docs/animation-smoothness.md`.
    ...
    /// position before writing. See "Mid-flight passes" in `docs/animation-smoothness.md`.
    ...
    fn animate_tile_movement(
        ...
        if !crate::window_snapshot::is_a_resize(from.size, to.size) {
    ...
    /// curve. See "Resizes through the overlay" in `docs/animation-smoothness.md`.
    fn animate_tile_resize(&mut self, window: WindowId, from: CGRect, to: CGRect, timing: Timing) {
        ...
        {
            let rect_at = |layout: &crate::edge_dressing::DressingLayout, index: usize| {
...
/// has to be captured on its own" in `docs/capture-overlay-research.md`.
...
/// during animations" in `docs/animation-smoothness.md`.
...
/// shadow covers the whole layer" in `docs/capture-overlay-research.md`.
...
mod tests {
    ...
    fn the_dressing_piece_set_does_not_depend_on_the_tile_size() {
        use crate::edge_dressing::EdgeDressing;
        use crate::window_snapshot::test_bitmap;
    ...
    fn full_dressing() -> crate::edge_dressing::EdgeDressing {
        use crate::window_snapshot::test_bitmap;
        let piece = || Some(test_bitmap());
        crate::edge_dressing::EdgeDressing {
            strips: [piece(), piece(), piece(), piece()],
            corners: [piece(), piece(), piece(), piece()],
        }
    }
