//! What the application knows about every window it has met: its AX state, its window-server
//! identity and visibility, where it sits in native fullscreen, and what the rules decided. Which
...
pub struct NativeFullscreenRecord {
    ...
    /// The native space of the workspace the window was assigned to when it went fullscreen.
...
impl WindowCatalogue {
    ...
    /// `assigned_space` is the native space of the window's workspace assignment, if it has one;
    /// the catalogue does not know assignments, so the caller passes it.
    ...
    /// A window in native fullscreen was (re)assigned: remember the space it now belongs to.
    pub fn note_native_fullscreen_assigned_space(&mut self, window_id: WindowId, space: SpaceId) {
...
mod tests {
    ...
    fn authoritative_space_only_record_is_not_pruned() {
        let mut window_store = WindowCatalogue::default();
    ...
    fn native_fullscreen_record_preserves_explicit_fallback_user_space_without_assignment() {
        ...
        assert_eq!(record.assigned_space, None);
    ...
    fn remove_window_preserves_native_fullscreen_record_until_app_cleanup() {
        ...
        window_store.insert_window(
            wid,
            WindowState::from(crate::app::WindowInfo {
                is_standard: true,
                is_root: true,
                is_minimized: false,
                is_resizable: true,
    ...
    fn rebinding_server_identity_keeps_both_indexes_unique() {
        let mut store = WindowCatalogue::default();
