...
enum WindowCommands {
    ...
    /// Fold the selected window into the column on this side, or back out to where it came from
    ToggleFold { direction: String },
...
enum SubscribeCommands {
    ...
    Mach {
        /// Event to subscribe to (workspace_changed, windows_changed, window_title_changed, focused_window_changed, *)
        event: String,
    },
...
fn map_window_command(cmd: WindowCommands) -> Result<CliCommand, String> {
    ...
    match cmd {
        ...
        WindowCommands::ToggleFold { direction } => Ok(CliCommand::Reactor(
            reactor::Command::Layout(LC::ToggleFold(parse_direction(&direction)?)),
        )),
...
fn parse_event_kind(input: &str) -> Result<EventKind, String> {
    match input.trim().to_ascii_lowercase().as_str() {
        ...
        "*" => Ok(EventKind::All),
        other => Err(format!(
            "Invalid event '{}'; expected workspace_changed, windows_changed, window_title_changed, focused_window_changed, or *",
            other
        )),
    }
}
