...
| `rini-wm` | the reactor: 5.9k lines plus `events/`, holding every context's state | the largest single job; it becomes a dispatcher over the contexts now that they exist to dispatch to. Done so far: the pure logic that had no reason to live there (transaction bookkeeping, manageability, space activation policy, screen selection, raise-echo and activation-focus rules, strip-stack and final-frame-write geometry) is in its context, and the two focus actors (`RaiseManager`, `MainWindowTracker`) run there against an `EventSink`/`FocusEvent` instead of the reactor's types; `AnimationManager` gathers a `PassWindow` per window and applies a `rini_animation::pass::PassPlan` rather than deciding per window inline; app-rule re-evaluation is `LayoutEngine::{before_rules, assign_window_by_rules, settle_app_rule}` with the follow-up decided by `AfterRules`, used by both the discovery and the app-rule paths; space resolution (which native space a window is on, from live read, pending frame, assignment or geometry) is `reactor/space_affinity.rs`, a read-only view over the three stores that stays in `rini-wm` because it spans windows, displays and workspaces; queries answer from `query::StateView`, a borrowed read-only snapshot of the stores, so a query can no longer mutate the engine; what remains is orchestration plus context state the reactor still owns |
