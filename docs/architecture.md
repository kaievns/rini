...
| `rini-workspaces` | next: the rest of `rini-layout` (engine, virtual workspaces, assignment, persistence, launch memory, floating positions, display affinity); then the `WindowStore` catalogue half moves to `rini-windows` |
