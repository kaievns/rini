...
| `src/animation/platform/engine.rs` | 815 of its 2.7k code lines are pure across 51 items — `should_swap_mid_flight`, `handover_report`, `companion_of`, `capture_work_allowed`, `merge_action`, `apply_frames_at` and nine pure enums — and 3.8k lines of tests point at them. Only 5 items touch macOS, and 1.6k lines are one `impl FlightEngine` | `animation/domain/` holds only `motion/` and `pass.rs`, so the feature's decision logic sits in the folder `tests/architecture.rs` exempts from rule 2. This is the largest remaining gap between this document and the tree. Moving the pure items is mechanical; splitting `impl FlightEngine` is not |
