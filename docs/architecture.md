...
| `rini-windows` | windows and the apps that own them | `WindowId`, `WindowServerId`, `pid_t`, `AppInfo`, `WindowInfo`, the window catalogue (`WindowStore`), app rules and what counts as manageable, frame transaction ids, the raise-echo and activation-focus rules; the per-app AX actor (observe, move, resize, raise, close), window-server reads (ids, order, levels), the Carbon front-app listener; app-rule and snapping settings. Emits `windows::Event` |
...
| `rini-animation` | movement on screen | flight plans, easing, z-bands, the workspace strip stack, which final frame writes go out; window snapshots and capture (ScreenCaptureKit, SkyLight), the Core Animation tile overlay, the flight engine; animation settings |
...
| `rini-wm` | the reactor: 6.2k lines plus `events/`, holding every context's state | the largest single job; it becomes a dispatcher over the contexts now that they exist to dispatch to. Done so far: the pure logic that had no reason to live there (transaction bookkeeping, manageability, space activation policy, screen selection, raise-echo and activation-focus rules, strip-stack and final-frame-write geometry) is in its context; what remains is orchestration plus context state the reactor still owns |
