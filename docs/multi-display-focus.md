...
- `RaiseEcho` sits earlier in the same handler and returns a raise's own focus
  ...
  times per press" in `crates/rini-overlay/docs/capture-overlay-research.md`.
...
`crates/rini-overlay/docs/capture-overlay-research.md`, "There is one overlay, so it follows the space
